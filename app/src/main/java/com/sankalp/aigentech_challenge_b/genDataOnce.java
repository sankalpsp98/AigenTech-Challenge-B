...
public class genDataOnce extends AppCompatActivity {
    ...
    private void ExecuteTask() {
        ...
        if (sharedPreferences12.getString("Email", "NA").equals("NA")) {
            ...
            WorkManager.getInstance(getApplicationContext()).beginWith(oneTimeWorkRequest2).enqueue();
            WorkManager.getInstance(getApplicationContext()).getWorkInfoByIdLiveData(oneTimeWorkRequest2.getId()).observe(this, new Observer<WorkInfo>() {
                @Override
                public void onChanged(WorkInfo listLiveData) {
            ...
            WorkManager.getInstance(getApplicationContext()).beginWith(oneTimeWorkRequest1).enqueue();
            WorkManager.getInstance(getApplicationContext()).getWorkInfoByIdLiveData(oneTimeWorkRequest1.getId()).observe(this, new Observer<WorkInfo>() {
                @Override
                public void onChanged(WorkInfo listLiveData) {
                    if (listLiveData != null && listLiveData.getState().isFinished()) {
                        new Handler().postDelayed(new Runnable() {
